    ...
    test(`No horizontal overflow on ${vp.name} (${vp.width}x${vp.height})`, async ({ page }) => {
      ...
      for (const route of routes) {
        ...
        if (hasOverflow) {
          ...
          console.error(`Route ${route} has overflow on ${vp.name}! ScrollWidth: ${scrollWidth}, InnerWidth: ${activeWidth}`);
        }
        expect(hasOverflow).toBe(false);
      }
    });

    test(`AdminTenantDetail handles long text without overflow on ${vp.name} (${vp.width}x${vp.height})`, async ({ page }) => {
      await page.setViewportSize({ width: vp.width, height: vp.height });
